...
public class ComputerManagerService extends Service {
                ...
                while (!isInterrupted() && pollingActive && tuple.thread == this) {
                    try {
                        ...
                        synchronized (tuple.networkLock) {
                            ...
                            if(tuple.computer != null) {
                                if(tuple.computer.state == ComputerDetails.State.ONLINE)
                                    tuple.computer.offlineCount = offlineCount;
                                else
                                    tuple.computer.offlineCount = 0;
                            }
                        }
    ...
    public class ComputerManagerBinder extends Binder {
        ...
        public void restartPolling(ComputerManagerListener listener) {
            ComputerDatabaseManager tempDbManager = null;
            try {
                stopPolling();
                tempDbManager = new ComputerDatabaseManager(ComputerManagerService.this);
                pollingTuples.clear();
                for (ComputerDetails computer : tempDbManager.getAllComputers()) {
    ...
    private ComputerDetails tryPollIp(ComputerDetails details, ComputerDetails.AddressTuple address) throws ComputerUuidChangedException {
        try {
            ...
            // details.uuid can be null on initial PC add
                ...
                if(details.machineIdentifier != null
                        && details.machineIdentifier.equals(newDetails.machineIdentifier)) {
                    LimeLog.warning("Same machine but "+details.name+" had it UUID changed. Maybe host reinstalled.");
                    throw new ComputerUuidChangedException(details, newDetails);
                }
                return null;
            }
    ...
    private void startParallelPollThread(ParallelPollTuple tuple, HashSet<ComputerDetails.AddressTuple> uniqueAddresses) {
        ...
        tuple.pollingThread = new Thread() {
            @Override
            public void run() {
                ComputerUuidChangedException uuidChangedException = null;
                ComputerDetails details = null;
                try {
                    details = tryPollIp(tuple.existingDetails, tuple.address);
                } catch (ComputerUuidChangedException e){
                    uuidChangedException = e;
                }
                synchronized (tuple) {
                ...
                if(uuidChangedException != null) {
                    onComputerUuidChanged(uuidChangedException.oldDetails, uuidChangedException.newDetails);
                }
            }
        };
    ...
    private void onComputerUuidChanged(@NonNull ComputerDetails oldDetails, @NonNull ComputerDetails newDetails) {
        ComputerManagerListener existingListener = listener;
        if(existingListener == null) {
            LimeLog.warning("onComputerUuidChanged: no existingListener");
            return;
        }
        if(!pollingActive) {
            LimeLog.warning("onComputerUuidChanged: pollingActive is false");
            return;
        }
        LimeLog.warning("onComputerUuidChanged: removeComputer");
        binder.removeComputer(oldDetails);
        LimeLog.warning("onComputerUuidChanged: notifyComputerRemoved "+oldDetails.uuid);
        if(existingListener instanceof ComputerManagerListener2){
            ((ComputerManagerListener2)existingListener).notifyComputerRemoved(oldDetails);
        }
        // will need to treat it as no server cert
        newDetails.serverCert = null;
        newDetails.activeAddress = oldDetails.activeAddress;
        LimeLog.warning("onComputerUuidChanged: addTuple "+newDetails.uuid);
        addTuple(newDetails);
        LimeLog.warning("onComputerUuidChanged: updateComputer "+newDetails.uuid);
        dbManager.updateComputer(newDetails);
        existingListener.notifyComputerUpdated(newDetails);
    }
