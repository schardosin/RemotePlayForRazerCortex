...
public class ControllerHandler implements InputManager.InputDeviceListener, UsbDriverListener {
    ...
    private boolean isRumbleWithNexus = false;
    ...
    public ControllerHandler(Activity activityContext, NvConnection conn, GameGestures gestures, PreferenceConfiguration prefConfig) {
        ...
        try {
            isRumbleWithNexus = NeuronBridge.isRumbleWithNexus().get();
            LimeLog.info("isRumbleWithNexus = "+ isRumbleWithNexus);
        } catch (Exception e) {
            NeuronBridge.logAndRecordException(e);
        }
    }
    ...
    private void rumbleDualVibrators(VibratorManager vm, short lowFreqMotor, short highFreqMotor) {
        ...
        // but... there are cases where this is is wrong, so we swap
        NeuronBridge.invertMotors(vibratorAmplitudes);
    ...
    public void handleRumble(short controllerNumber, short lowFreqMotor, short highFreqMotor) {
        ...
        if (isRumbleWithNexus) {
            int normalizedHighFreqMotor = (short)((highFreqMotor >> 8) & 0xFF);
            int normalizedLowFreqMotor = (short)((lowFreqMotor >> 8) & 0xFF);
            LimeLog.info("isRumbleWithNexus: controllerNumber=" + controllerNumber + " of ("+inputDeviceContexts.size()+") lowFreqMotor=" + lowFreqMotor + ", highFreqMotor=" + highFreqMotor + ", normalizedLowFreqMotor=" + normalizedLowFreqMotor + ", normalizedHighFreqMotor=" + normalizedHighFreqMotor);
            if(controllerNumber == 0 || inputDeviceContexts.size() == 1) {
                NeuronBridge.doVibrate(normalizedLowFreqMotor, normalizedHighFreqMotor);
                return;
            }
        }
